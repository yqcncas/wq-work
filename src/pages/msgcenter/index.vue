      ...
      <div class="bottom">
        ...
        <div class="input">
          ...
          <form report-submit='true' @submit='getFormID' class="form">
            <button form-type="submit" @click="sendMessage(userId)" size="mini" :loading="sendLoad" :disabled="sendLoad">发送</button>
          </form>
      </div>
      <div class="word-box">
...
export default {
  ...
  methods: {
      ...
      if (this.type === 'sale') {
        this.$fly.request({
          ...
          if (code === 200) {
            ...
            result.pageInfo.list.forEach(e => {
              this.msgforuser.unshift(e)
            })
          }
        }).catch(err => {
    ...
    sendMessage () {
      ...
      let data = { cmd: 'chat', fromUserId: this.selfId, toUserId: this.userId, msg: { msg: this.paramStrs } }
  ...
  onLoad (options) {
    ...
    this.getMessage({ pageNum: 1, mode: 1 })
    this.paramStrs = ''
  },
  onShow () {
    this.getList()
